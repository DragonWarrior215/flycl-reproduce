...
class Trainer(object):
    ...
    def _init_device(self, config):
        ...
        # CPU fallback: the upstream framework assumes CUDA, but analytic methods
        # (Fly-CL, RanPAC, NCM) run fine on CPU. Honour an explicit cpu request or
        # the absence of a usable GPU rather than crashing on torch.cuda.set_device.
        if not torch.cuda.is_available() or config.get("device_ids") in ("cpu", ["cpu"]):
