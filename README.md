...
- 端到端墙钟（含真 ViT 前向的特征提取 + 闭式解，单卡 4090）：CIFAR 348 s、CUB 203 s、VTAB 101 s。
...
Fly-CL 是闭式线性代数，抽好特征后在 CPU 上秒级可解。§1.2 的方法对比与 Level-2 分析走的就是这条特征缓存路径。
CPU 环境的坑（MKL 版本、OpenMP 绑核）见 [§12](#12-踩坑记录)。
...
论文预处理是确定性变换，缓存特征与在线前向数值等价。§1.2 的基线对比和 Level-2 分析走这条路。
...
原仓库里的 LibContinual 子集缺了几样东西，Fly-CL 之所以此前只能走特征缓存脚本、跑不了 `run_trainer.py`，
就是卡在这里。本次补齐：
...
| `libtorch_cpu.so: undefined symbol: iJIT_NotifyEvent` | CPU 路径下 conda 装了 MKL 2025，torch 2.0.1 依赖的符号被移除 | `conda install -c conda-forge mkl==2024.0.0`                                            |
